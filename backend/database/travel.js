...
const get2 = (travel_id) => {
  return new Promise(async (resolve, reject) => {
    var sqls = [
      "SELECT T.travel_id,T.name AS travel_name,T.date,T.people_num,T.description AS travel_description,T.done,T.group_id,S.spot_id,S.name,S.location,S.ranking,S.open_hour,S.description,H.arrive_time,H.start_time,H.transportation, G.tag_id,G.name, G.color FROM TRAVEL AS T INNER JOIN HAS AS H ON T.travel_id = H.travel_id INNER JOIN SPOT AS S ON H.spot_id = S.spot_id LEFT JOIN TAG AS G ON H.tag_id = G.tag_id where T.travel_id=? ",
    ];
